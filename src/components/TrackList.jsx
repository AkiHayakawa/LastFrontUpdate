import React from "react";
import NavbarSpotify from "./NavbarSpotify";
// import NavbarSpotify from "./components/NavbarSpotify";

const TrackList = () => {
  return (
<>
{/* <NavbarSpotify/> */}
    
    <div style={{display:'inline'}}>
      <div className="GoodMorning">
        <h2>Good evening</h2>
        <div style={{ display: "flex", flexWrap: "wrap" }}>
          <div className="evening">
            <img
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
              className="eveningImg"
            />
            <h3 style={{ marginLeft: "15px" }}>mix house</h3>
          </div>
          <div className="evening">
            <img
              src="https://lite-images-i.scdn.co/image/ab67706f00000002ca5a7517156021292e5663a6"
              alt=""
              className="eveningImg"
            />
            <h3 style={{ marginLeft: "15px" }}>mix house</h3>
          </div>
          <div className="evening">
            <img
              src="https://i.ytimg.com/vi/hTWKbfoikeg/maxresdefault.jpg"
              alt=""
              className="eveningImg"
            />
            <h3 style={{ marginLeft: "15px" }}>mix house</h3>
          </div>
          <div className="evening">
            <img
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
              className="eveningImg"
            />
            <h3 style={{ marginLeft: "15px" }}>mix house</h3>
          </div>
          <div className="evening">
            <img
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
              className="eveningImg"
            />
            <h3 style={{ marginLeft: "15px" }}>mix house</h3>
          </div>
          <div className="evening">
            <img
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
              className="eveningImg"
            />
            <h3 style={{ marginLeft: "15px" }}>mix house</h3>
          </div>
        </div>
      </div>
      <div>
        <h2>Made for Guilherme Dourado</h2>
        <div className="List">
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>
        </div>
      </div>{" "}
      {/* <div>
        <h2>Made for Guilherme Dourado</h2>
        <div className="List">
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>
        </div>
      </div>{" "}
      <div>
        <h2>Made for Guilherme Dourado</h2>
        <div className="List">
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>
        </div>
      </div>{" "}
      <div>
        <h2>Made for Guilherme Dourado</h2>
        <div className="List">
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>{" "}
          <a className="ListBlock" href="#">
            <img
              className="ListImg"
              src="https://images.ctfassets.net/bdyhigkzupmv/6lySzcy7qcIN1tf81Qkus/5b5ac73daeaf61f9057c0b0dd8447a31/hero-guitar-outro.jpg"
              alt=""
            />
            <h3 style={{ margin: "10px" }}>Daily for 2</h3>
            <h5 style={{ margin: "10px" }}>
              Drake, Tyler, The Creator, Kanye West and more
            </h5>
          </a>
        </div>
      </div> */}
    </div>
    </>
  );
};

export default TrackList;
